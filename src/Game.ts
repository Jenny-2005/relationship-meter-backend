...
type Question = {
  id: number;
  text: string;
  moveAnswers: Answer[];   // answers that cause "move-away"
};
...
export class Game {
  ...
  private positions: Map<WebSocket, number>;
  private answers: Map<WebSocket, Answer>;
  ...
  // HANDLE ANSWER
  // ------------------------------------------------------------------
  makeMove(socket: WebSocket, answer: Answer) {
    this.answers.set(socket, answer);

    // wait until both have answered
    if (this.answers.size < 2) return;
